...
export const TOOLS = [
  ...
  { category: "read", permissionClass: "requires_confirmation", logsInputs: true, def: { type: "function", function: { name: "long_job", description: "Promote a big ask into a LONG-RUN JOB that keeps working on the server for hours (even overnight, even with the app closed) with a budget fuse: spend pauses the job at each approved tranche instead of running away. Use when the user asks for something too large for one reply (review a whole app, write many chapters, process a long list) or says overnight/keep going/long job. Creating a job SPENDS REAL MONEY as it runs, so state the mission and budget before creating. Also checks status, pauses, and resumes jobs.", parameters: { type: "object", properties: { action: { type: "string", enum: ["create", "status", "pause", "resume"], description: "create needs mission+plan+model; status/pause/resume take an id (status without id lists jobs)." }, mission: { type: "string", description: "One sentence: what the finished job delivers." }, plan: { type: "array", items: { type: "object", properties: { title: { type: "string" }, detail: { type: "string" } }, required: ["title"] }, description: "The work cut into small verifiable units, in order. Each unit should be one bounded step." }, model: { type: "string", description: "A cloud model id from this app's catalog to drive the units." }, tranches: { type: "number", description: "How many budget tranches to approve up front (guest tranche $1, max 10; owner tranche $5)." }, id: { type: "string", description: "Job id for status/pause/resume." } }, required: ["action"] } } } },
  ...
  { category: "document", permissionClass: "draft_only", logsInputs: true, def: { type: "function", function: { name: "save_plan", description: "Save a BUILD PLAN this conversation produced (a roadmap, phase breakdown, task list, or MVP definition for an app or project) to the plan library. When you have laid out a complete plan of any of those kinds, OFFER to save it, and call this when the user agrees. Saved plans can be revisited, downloaded, reloaded here to edit, or opened in The Crucible to start a project. Returns the plan id. Use this instead of create_artifact whenever the document is a plan for something to be built.", parameters: { type: "object", properties: { title: { type: "string", description: "A short project name for the plan." }, content: { type: "string", description: "The full plan text (markdown: phases, tasks, MVP scope)." }, tags: { type: "array", items: { type: "string" } } }, required: ["title", "content"] } } } },
...
export async function runTool(name, args, ctx, signal = null) {
  ...
  try {
    ...
    switch (name) {
      ...
      // Long-run jobs: the server injects ctx.longJob per turn (money gates live there).
      case "long_job": return ctx.longJob ? String(await ctx.longJob(args)) : "Long-run jobs are not available right now.";
      // Plan Pipeline (Phase 2A): save a build plan the conversation produced, so it can be
      // revisited, reloaded to edit, or sent to the Crucible to start a project. It is a normal
      // artifact of type "plan", so download/history/versioning all come for free.
      case "save_plan": {
        if (!ctx.artifacts) return "The plan library isn't available right now.";
        const prov = typeof ctx.provenance === "function" ? ctx.provenance() : {};
        const r = ctx.artifacts.create({ title: args.title || "Untitled plan", type: "plan", content: args.content,
          tags: ["plan", ...(Array.isArray(args.tags) ? args.tags : [])], model: ctx.model || "", sourceChatId: prov.sourceChatId,
          sourceContextRefs: prov.sourceContextRefs, sourceToolRunIds: prov.sourceToolRunIds, promptSummary: prov.promptSummary });
        if (r && r.error) return "Couldn't save the plan: " + r.error;
        const id = (r && (r.id || (r.artifact && r.artifact.id))) || "";
        return "Saved the plan \"" + (args.title || "Untitled plan") + "\"" + (id ? " (id " + id + ")" : "") +
          ". It's in the plan library: open The Crucible and choose it under \"Start from a saved plan\" to turn it into a project, or reload it here any time to edit.";
      }
