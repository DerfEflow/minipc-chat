 ...
 *
 * Four collections in one zero-dep JSON store: the failure/hallucination LEDGER, EVAL cases +
 * eval RUNS, and prompt RULES. Mentor critiques and "save lesson" actions feed these; active
 * prompt rules get injected into prompts; eval runs measure whether changes actually help.
...
export function createFlywheel(opts = {}) {
  ...
  let db = { failures: [], evals: [], runs: [], rules: [] };
  const load = () => { try { if (existsSync(file)) { const j = JSON.parse(readFileSync(file, "utf8")); if (j && typeof j === "object") db = { failures: j.failures || [], evals: j.evals || [], runs: j.runs || [], rules: j.rules || [] }; } } catch {} };
  ...
  function addRule(e = {}) {
    ...
    const it = { id: randomUUID(), scope: ["global", "mode", "tool", "mentor", "router", "workspace"].includes(e.scope) ? e.scope : "global", content: S(e.content, 1000), sourceEvalId: e.sourceEvalId || null, status: ["candidate", "active", "retired"].includes(e.status) ? e.status : "candidate", createdAt: nowIso() };
  ...
  // ---- generic ops ----
  const COLLS = new Set(["failures", "evals", "runs", "rules"]);
  ...
  const stats = () => ({ failures: db.failures.length, evals: db.evals.length, runs: db.runs.length, rules: db.rules.length, openFailures: db.failures.filter((f) => f.status === "open").length, activeRules: db.rules.filter((r) => r.status === "active").length });

  return { addFailure, addEval, addRun, addRule, activeRules, update, remove, list, get, runsFor, stats };
}
