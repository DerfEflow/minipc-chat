...
(() => {
  ...
  const DICT = {
    ...
    st_skipped: ["Skipped", "Skipped", "Skipped (a task it needed failed)"],
    ...
    af_plan_spent: ["planning has cost", "planning spend:", "planning spend so far:"],
    af_plan_parallel: ["tasks that can run together, will", "parallel waves", "parallel waves (independent tasks run together)"],
    af_plan_serial: ["if run one at a time", "sequential", "sequential (one task at a time)"],
    af_furnace_ask: [
      "Ask me before fixing what the final honesty check finds (normally it just fixes them)",
      "Ask before auto-fixing Furnace findings",
      "Ask before fixing Furnace findings (the final honesty check; normally auto-fixes)",
    ],
    ...
    // Blueprints: 49 ready-made project shapes, so a build can start from something rather than a
    // blank interview. Plain register never says "blueprint" or "fan-out"; it says what they do.
    bp_open: ["Start from a ready-made plan", "Start from a blueprint", "Start from a blueprint (a ready-made plan)"],
    bp_title: ["Ready-made plans", "Blueprints", "Blueprints (ready-made plans)"],
    bp_sub: [
      "Pick whichever is closest to what you want. Nothing is locked in: you can change any part of it by talking.",
      "Pick the closest shape. Every part stays editable in the interview.",
      "Pick the closest shape. Nothing is locked in: every part stays editable in the interview.",
    ],
    bp_close: ["Close", "Close", "Close"],
    bp_back: ["Back to the list", "Back", "Back to the list"],
    bp_loading: ["Getting the plans...", "Loading blueprints...", "Loading blueprints..."],
    bp_failed: ["The plans could not be loaded. Try again in a moment.", "Blueprint catalog unavailable.", "Blueprint catalog unavailable. Try again in a moment."],
    bp_steps: ["stages", "steps", "steps"],
    bp_gate: [
      "Stops and asks you before it does anything outside the app",
      "Human approval before the acting step",
      "Human approval before the acting step (it stops and asks you first)",
    ],
    bp_needs: ["What it needs to start", "Inputs", "Inputs (what it needs to start)"],
    bp_makes: ["What it produces", "Outputs", "Outputs (what it produces)"],
    bp_works: ["How you will know it worked", "Success metrics", "Success metrics (how you will know it worked)"],
    bp_talk: ["Talk it through first", "Seed the interview", "Seed the interview (talk it through first)"],
    bp_asis: ["Use this plan as it is", "Use as-is", "Use as-is (skip the interview)"],
    bp_chose: ["Starting from a ready-made plan", "Blueprint applied", "Blueprint applied"],
