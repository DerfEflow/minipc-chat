...
export const CRUCIBLE_GUIDE = [
  ...
  "- Full Custom, in the same AF window: press Plan the sections and the planner splits your",
  "  brief into sections. Each section becomes a row where you pick ANY model and how many",
  "  helpers, with a live estimate of time, tokens and cost beside it that updates as you",
  "  choose. Any pick is allowed; a red note warns when a model is too small for a section",
  "  (expect truncation) but never blocks you, it is yours to experiment with. Only the",
  "  divider is held to non-tiny models. Every build runs on its own branch so your main",
  "  stays clean, and if a section fails the finished work is saved on that branch, never lost.",
