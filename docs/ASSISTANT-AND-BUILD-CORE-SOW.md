...
| Local Qwen removed from the lineup | Already absent from the catalog. No local entry is offered to anyone in the picker. **The Ollama transport is NOT dead weight and must NOT be swept. See the correction below.** |

> **CORRECTION, 2026-08-03, proven by lane A and confirmed by the integrator.** The line above used to call the Ollama transport "dead weight to sweep" and that was wrong. Fred said the local Qwen model was removed from the lineup, and the lineup means the model PICKER. The execution path never left. `ollamaChat()`, `PROVIDERS.light` and `PROVIDERS.main`, `LIGHT_MODEL` and `MAIN_MODEL`, `model:"local"`, the `/ollama` proxy, and `hands.mjs`'s `ollama_chat` tool are Fred's owner-only zero-egress lane for Command Deck and Private-mode work. It is also the keyless test fixture that at least five suites boot against. `autoroute_test.mjs` pins both halves of this: "owner AUTO resolves to a CLOUD model and never touches the local Qwen" and "explicit LOCAL still runs on the local Qwen and streams", and both pass today. `multitenant_e2e_test.mjs` pins the proxy as owner-only. Deleting this would have removed a working private lane and broken the test harness at the same time. The embed side on `ollamaEmbedText` was never in question and `server.mjs` states plainly that it stays on Ollama deliberately.
...
1. ~~Sweep the dead Ollama transport out of `server.mjs`.~~ **CANCELLED 2026-08-03. The transport is live, and the sweep would have caused an outage rather than a cleanup.** Lane A was dispatched to do this, read the code, ran the suites, and refused with evidence. The full finding is recorded in the corrected table row above and in `docs/wiring/lane-a-ollama.md`, which is written as a do-not-remove list. The only real item under this heading was `wolfe-logic.mjs`'s EMBER comment saying "including local Qwen", and lane E corrected that against the catalog while paring the file. The lesson worth keeping: this SOW sized the job at roughly 130 references across 26 files and never asked whether any of them were load-bearing. Reference counts measure effort. They say nothing about whether the work should happen.
