...
function createChatJob() {
  gcChatJobs();
  // uid is set as soon as identity resolves. The Fire Alarm uses it to scope a guest's kill to
  // their own turns, so one guest can never stop another guest's work or Fred's.
  const job = { id: "job_" + randomUUID().slice(0, 12), chatId: "", uid: "", startedAt: Date.now(), endedAt: 0,
...
async function handleChat(req, res) {
  ...
  job.chatId = chatId;
  job.uid = T.uid || "";   // Fire Alarm scoping
