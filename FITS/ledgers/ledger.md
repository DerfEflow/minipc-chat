...
| L-010 | 2026-08-05 | RISK-DECISION | Staleness is itself a refusal, so the shipped probe defaults expire after 7 days and would have silently stripped every measured seat from the guest picker. Caught in final review, not by a test. | Would have looked like "the free models disappeared" with no error and no deploy | EXECUTOR | RESOLVED(built `ops/seat-probe.mjs` + volume overlay + `seatprobe_test.mjs` asserting both exist) |
| L-011 | 2026-08-05 | MUST-VERIFY | `ops/seat-probe.mjs` has never been executed. It is written against the same streaming shape the manual sweep used, but running it from the container is unverified. | If it errors, probes never refresh and L-010 comes back in 7 days | USER | OPEN — run `railway ssh "node ops/seat-probe.mjs --dry"` |
| L-012 | 2026-08-05 | RISK-DECISION | Nothing SCHEDULES the probe. Fred must run it, or it needs a cron/watchdog hook. | Same 7-day expiry, just deferred | USER | OPEN |
| L-013 | 2026-08-05 | MUST-VERIFY | The resumable attach is proven at the `idejobs.attach` layer and by source assertion on the client. It has NOT been exercised through a real dropped Cloudflare stream in a browser. | A wrong cursor would skip events rather than replay them, which is a worse failure than the one fixed | USER | OPEN — watch one live build through a drop |
| L-014 | 2026-08-05 | ASSUMPTION | `state.events.length` is a correct cursor into the server's `job.events`, because the client pushes every delivered event exactly once. Falsifiable: any client-side filter that drops an event before the push would desync it. Today only `type:"gone"` returns early, and it is terminal. | Skipped events in the build view after a drop | EXECUTOR | OPEN |
